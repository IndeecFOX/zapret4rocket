...
rc=$?
if [ "$rc" = "130" ]; then
  echo -e "${yellow}Операция переустановки zapret отменена. Возврат в меню...${plain}"
  pause_enter   # опционально
  get_menu      # запускаем главное меню
  exit 0        # чтобы не пошёл дальше код “тела” после меню
fi
