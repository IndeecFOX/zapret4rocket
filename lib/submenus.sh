...
strategies_submenu() {
  while true; do
    ...
    submenu_item "	2" "YouTube (TCP. Интерфейс)." "22 варианта (Приоритетнее безразборного режима)"
    submenu_item "	3" "YouTube (TCP. Видеопоток/GV домен)." "22 варианта (Приоритетнее безразборного режима)"
    submenu_item "	4" "RKN (Популярные блокированные сайты. Дискорд в т.ч.)." "22 варианта"
    submenu_item "	5" "Отдельный домен." "22 варианта (Приоритетнее безразборного режима)"
    if [ -n "$CONFIG_UPDATE_NOTICE" ]; then
